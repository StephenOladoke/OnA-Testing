...
function fetchData() {
  ...
  fetch(
    `https://onabooking-api.onrender.com/api/v1/properties?location=${query}`
  )
    .then((response) => {
      if (!response.ok) {
...
function displayGalleryProperties(data) {
  const propertyContainer = document.getElementById("galleryContainer");
